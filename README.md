...
首頁加入八日滾動旅程：四城鐵道路線、小火車轉場、日期跳轉、城市照片明信片、可展開時間表與每日導航。行程沿用 `src/data/trip.js`，不另外複製時刻；路線為城市間的示意連線，非實際鐵路軌跡。Auschwitz 與博物館章節採靜態文字呈現。

模板為 `src/templates/journey.mjs`，動畫為 `src/scripts/journey.js`，樣式為 `src/styles/journey.css`。建置時合併樣式，單檔版內嵌動畫，離線快取也包含動畫程式。可暫停動態並記住本分頁的偏好；自動尊重系統減少動態設定。沒有 JavaScript 時仍可閱讀全部章節、展開時間表和使用日期錨點。

### 動畫與既有介面的隔離（2026-09-12）

- 動畫移到原有八日目錄後，以原生 details 預設收合；封面入口可直接展開，不擋住待辦、行程與城市指南。
- 手機或低高度橫向螢幕預設減少動態；使用者仍可主動啟用，系統減少動態設定優先。手機地圖不固定、不蓋住導覽與時間表，照片不做傾斜轉場。
- 白天遊覽城市與本日路線終點分開顯示，避免跨城日與每日景點地圖混淆。歷史章節不播放火車轉場。
- 收合、離開動畫區、背景分頁時停止火車動畫；動畫不接管觸控手勢或全站捲動。
- 動畫程式僅首頁載入；單檔版在其他章節隱藏時略過行程量測。仍待實機視覺驗收。
